...
TechCorp Solutions enfrenta actualmente problemas críticos en su proceso de desarrollo: despliegues manuales propensos a errores que generan tiempos de entrega de hasta 3 semanas, falta de visibilidad en la calidad del código que resulta en bugs recurrentes en producción, ausencia de pruebas automatizadas que compromete la confiabilidad del sistema, y silos organizacionales que dificultan la colaboración entre desarrollo y operaciones. Para abordar estos desafíos, este proyecto entregará una solución integral que incluye: (1) una aplicación web de gestión empresarial con autenticación y operaciones CRUD como caso de uso demostrador, (2) un pipeline CI/CD completamente automatizado en GitHub Actions que reduce el tiempo de despliegue a minutos, (3) infraestructura contenerizada con Docker que garantiza consistencia entre entornos, (4) integración de herramientas de calidad (SonarQube con cobertura >80%, Flyway para versionado de BD), (5) automatización de pruebas en múltiples niveles (unitarias, API con Newman, funcionales con Selenium, rendimiento con JMeter), (6) gestión centralizada de artefactos con Nexus, y (7) un modelo organizativo DevOps basado en squads que fomenta la colaboración. El éxito se medirá mediante la evaluación del nivel de madurez DSOOM y el análisis de Value Stream Mapping que cuantificará la reducción de desperdicios y la mejora en el tiempo de ciclo de desarrollo.
